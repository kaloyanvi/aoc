    ...
    for line in lines:
        ...
        if contained_within[1] != " no other bags.":  # bags that can contain other bags
            ...
            for bag in contained_bags:
                ...
                contained_bag["count"] = int(bag.strip()[0])
...
def findBag2(starting_bag, container_big_data):
    count = 0
    for bag in container_big_data[starting_bag]:  # iterate over the values of the input bag
        count += bag["count"] + bag["count"] * findBag2(bag["type"], container_big_data)  # recursively go deeper

    return count

print("Answer %i"%findBag2("shiny gold", container_bag_data))